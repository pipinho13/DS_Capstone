...
PredNextTerm <- function(inStr)
{
  ...
  if (inStrLen >= 3 & !nxtTermFound)
  {
    ...
    if ( length(fDF4Temp[, 1]) > 1 )
    {
      ...
      nxtTermFound <- TRUE;
      wcloud<-fDF4Temp;
      probtable<-data.frame(fDF4Temp, prop=round(100*fDF4Temp$freq/sum(fDF4Temp$freq),2));
      probtable<-data.frame(NextWord=head(word(probtable$terms,-1)), Probability=head(probtable$prop));
  ...
  if (inStrLen >= 2 & !nxtTermFound)
  {
    ...
    if ( length(fDF3Temp[, 1]) > 1 )
    {
      ...
      nxtTermFound <- TRUE;
      wcloud<-fDF3Temp;
      probtable<-data.frame(fDF3Temp, prop=round(100*fDF3Temp$freq/sum(fDF3Temp$freq),2));
      probtable<-data.frame(NextWord=head(word(probtable$terms,-1)), Probability=head(probtable$prop));
  ...
  if (inStrLen >= 1 & !nxtTermFound)
  {
    ...
    if ( length(fDF2Temp[, 1]) > 1 )
    {
      ...
      nxtTermFound <- TRUE;
      wcloud<-fDF2Temp;
      probtable<-data.frame(fDF2Temp, prop=round(100*fDF2Temp$freq/sum(fDF2Temp$freq),2));
      probtable<-data.frame(NextWord=head(word(probtable$terms,-1)), Probability=head(probtable$prop));
  ...
  if (!nxtTermFound & inStrLen > 0)
  {
    ...
    wcloud<-fDF1;
    probtable<-data.frame(fDF1, prop=round(100*fDF1$freq/sum(fDF1$freq),2));
    probtable<-data.frame(NextWord=head(word(probtable$terms,-1)), Probability=head(probtable$prop));
  ...
  if (inStrLen > 0){
    dfTemp1 <- list(nextTerm=nextTerm, mesg=mesg, wcloud=wcloud, probtable=probtable);
    return(dfTemp1);
  } else {
    nextTerm <- "";
    probtable<-"";
...
shinyServer(function(input, output) {
  output$prediction <- renderPrint({
    ...
    input$action;
    msg<<-as.character(strDF$mesg);
    cat("", as.character(strDF$nextTerm))
    cat("\n\t");
    cat("\n\t");
    cat("Note: ", as.character(strDF$mesg));
    ...
    output$plot <- renderPlot({
      ...
      wordcloud_rep(word(strDF$wcloud$terms,-1), strDF$wcloud$freq, scale=c(5,0.5), max.words=20,
                    colors=brewer.pal(8, "Dark2"))
    })
    
    output$plot2 <- renderPlot({
      
      wordcloud_rep((strDF$wcloud$terms), strDF$wcloud$freq, scale=c(4,0.5), max.words=20,
                    colors=brewer.pal(8, "Dark2"))
    })
